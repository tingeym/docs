# Source: https://docs.fluid.app/docs/themes/vox_theme_configuration

# Vox Theme Configuration System Documentation

## Overview

The Vox theme uses a comprehensive configuration system that allows users to customize the theme's appearance and behavior through a structured JSON-based settings system. This document explains how the configuration files work together to create a customizable, maintainable theme.

## Architecture Overview

The Vox theme configuration system consists of four main components:
...
### Configuration Groups in Vox Theme

1. **`typography`** - Font families, sizes, weights
2. **`spacing`** - Spacing scale (xs, sm, md, lg, xl, 2xl-10xl)
3. **`color_schema`** - Color palette (primary, neutral, gradients)
...
.btn {
 ...
 border-radius: var(\--btn-rounded-5xl);
...
The theme uses a consistent pattern for color variations:

- Base: `color_primary` → `--clr-primary`
- With opacity: `color_primary_400` → `--clr-primary-400`
- Shades: `color_black_500`, `color_black_600`, etc.
...
The Vox theme configuration system provides a powerful, flexible way to customize the theme:
